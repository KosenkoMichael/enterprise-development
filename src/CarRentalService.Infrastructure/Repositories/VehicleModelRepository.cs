...
public class VehicleModelRepository(CarRentalDbContext dbContext) : IRepository<VehicleModel>
{
    ...
    {
        var result = await dbContext.VehicleModels.Where(x => x.Id == id).ExecuteDeleteAsync();
        return result > 0;
    }
    /// <inheritdoc/>
    public async Task<List<VehicleModel>> ReadAllAsync() =>
        await dbContext.VehicleModels.ToListAsync();
    /// <inheritdoc/>
    public async Task<VehicleModel?> ReadAsync(Guid id) =>
        await dbContext.VehicleModels.FirstOrDefaultAsync(x => x.Id == id);
    /// <inheritdoc/>
